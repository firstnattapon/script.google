...
function saveToSheet(data) {
  try {
    ...
    // สร้าง Sheet ใหม่ถ้ายังไม่มี พร้อมหัวตารางที่เพิ่มช่อง LINE ID
    if (!sheet) {
      sheet = ss.insertSheet(sheetName);
      sheet.appendRow(["วันที่และเวลา", "ชื่อ-นามสกุล", "เบอร์โทร", "ชื่อ LINE/LINE ID", "อาชีพ", "จำนวนคน", "สัตว์เลี้ยง", "รถยนต์", "การย้ายเข้า", "คะแนนรวม", "สถานะ"]);
      sheet.getRange("A1:K1").setFontWeight("bold").setBackground("#d9ead3");
    }
    
    // บันทึกข้อมูล (ผูกข้อมูล LINE ID ลงตารางระบบกรอง)
    sheet.appendRow([
      new Date(), 
      data.name, 
      data.phone, 
